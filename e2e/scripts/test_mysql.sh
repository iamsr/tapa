...
OUTPUT=$("$TAPA_BIN" analyze "$E2E_DIR/fixtures/mysql_test_migration.sql" --db-type mysql --dry-run --format json 2>&1)
if [ $? -ne 0 ]; then
...
OUTPUT=$("$TAPA_BIN" analyze "$E2E_DIR/fixtures/mysql_test_migration.sql" --db-type mysql --db "$MYSQL_URL" --format json 2>&1)
if [ $? -ne 0 ]; then
    echo -e "${RED}    ✗ Failed: $OUTPUT${NC}"
    exit 1
fi
...
# Test 3: Check for MySQL-specific features (ALGORITHM, LOCK detection)
echo -e "${YELLOW}  Test 3: Verify MySQL-specific features${NC}"
...
# Test 6: Verify pt-online-schema-change recommendations for high-risk ops
echo -e "${YELLOW}  Test 6: Verify pt-osc recommendations${NC}"
